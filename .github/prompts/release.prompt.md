...
5. **Test** — Run `npm run test` and confirm tests pass.
6. **Build** — Run `npm run build` and confirm it succeeds.
7. **Deploy** — Copy `dist/main.js`, `manifest.json`, and `src/styles.css` to the test vault at `C:\Users\kevin\SynologyDrive\Obsidian Vault\TTRPG Vault\.obsidian\plugins\dnd-campaign-hub\`.
...
12. **Create GitHub release** — Run `gh release create vX.Y.Z --title "vX.Y.Z" --notes-file CHANGELOG.md release/main.js release/manifest.json release/styles.css`. If `gh` is not on PATH, use the full path `"C:\Program Files\GitHub CLI\gh.exe"`.
13. **Report** — Tell the user the release is published and link to the GitHub release page.
