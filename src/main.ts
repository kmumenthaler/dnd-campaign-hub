  ...
  async initializeVault() {
	...
	async renderMapView(source: string, el: HTMLElement, ctx: any) {
		try {
			...
			if (!config.mapId) {
				el.createEl('div', { 
					text: 'âš ï¸ Map configuration missing mapId',
					cls: 'dnd-map-error'
				});
				return;
			}
			...
			if (!config.imageFile) {
				el.createEl('div', { 
					text: 'âš ï¸ Map data not found. Please recreate the map.',
					cls: 'dnd-map-error'
				});
				return;
			}
			...
			if (!imageFile || !(imageFile instanceof TFile)) {
				el.createEl('div', { 
					text: `âš ï¸ Image file not found: ${config.imageFile}`,
					cls: 'dnd-map-error'
				});
				return;
			}
			...
			if (config.name) {
				...
				const typeEmoji = config.type === 'battlemap' ? 'âš”ï¸' : config.type === 'world' ? 'ðŸŒŽ' : 'ðŸ—ºï¸';
				info.textContent = `${typeEmoji} ${config.type} â€¢ ${config.dimensions.width}Ã—${config.dimensions.height}px`;
				
				if (config.scale) {
					const scale = titleBar.createEl('span', { cls: 'dnd-map-scale' });
					scale.textContent = `ðŸ“ ${config.scale.value} ${config.scale.unit} per square`;
				}
			}
    ...
    let activeTool: 'pan' | 'select' | 'highlight' | 'draw' | 'ruler' | 'target-distance' | 'eraser' | 'move-grid' | 'marker' | 'aoe' | 'fog' | 'walls' | 'lights' | 'walllight-draw' | 'elevation-paint' | 'difficult-terrain' | 'player-view' | 'poi' | 'terrain-paint' | 'climate-paint' | 'hexcrawl-move' | 'set-start-hex' | 'hex-desc' | 'magic-wand' | 'env-asset' = 'pan';
		// Background editing view â€” controls which element type is prominent and interactable
      ...
      let isMovingGmRect = false; // True when dragging existing rect to move it
		...
		// â”€â”€ Environment Asset state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
		...
		let lightDragOrigin: { x: number; y: number } | null = null;
			...
			const hexDistance = (c1: number, r1: number, c2: number, r2: number): number => {
				if (config.gridType === 'hex-horizontal') {
					// Offset (odd-col) hex grid â†’ convert to cube coordinates
					const x1 = c1;
					...
					// hex-vertical â†’ offset (odd-row) hex grid â†’ convert to cube coordinates
			...
			const LIGHT_SOURCES = {
				candle: { name: 'Candle', bright: 5, dim: 5, icon: 'ðŸ•¯ï¸', flicker: true },
				torch: { name: 'Torch', bright: 20, dim: 20, icon: 'ðŸ”¥', flicker: true },
				lantern: { name: 'Lantern', bright: 30, dim: 30, icon: 'ðŸ®' },
				bullseye: { name: 'Bullseye Lantern', bright: 60, dim: 60, cone: true, icon: 'ðŸ”¦' },
				light: { name: 'Light Spell', bright: 20, dim: 20, icon: 'âœ¨' },
				dancing: { name: 'Dancing Lights', bright: 0, dim: 10, icon: 'ðŸ’«', flicker: true },
				continual: { name: 'Continual Flame', bright: 20, dim: 20, icon: 'ðŸ”¥', flicker: true },
				daylight: { name: 'Daylight Spell', bright: 60, dim: 60, icon: 'â˜€ï¸' },
				fluorescent: { name: 'Fluorescent', bright: 30, dim: 10, icon: 'ðŸ’¡', flicker: true },
				walllight: { name: 'Wall Light', bright: 15, dim: 15, icon: 'ðŸ“', isLine: true }
			} as const;
			type LightSourceType = keyof typeof LIGHT_SOURCES;
			
			// â”€â”€ Light Flicker Animation System â”€â”€
			...
			const FLICKER_FPS = 14; // ~14 fps for flickering â€” smooth enough without being expensive
			...
			// Animation loop â€” only runs when flickering lights are present
			...
			const getWallHeightSummary = (indices: number[], walls: any[]): string => {
				...
				for (const wi of indices) {
					const w = walls[wi];
					if (!w) continue;
					const key = w.height !== undefined && w.height !== null ? `${w.height} ft` : 'âˆž (infinite)';
			...
			const showWallHeightPopup = (indices: number[]) => {
				...
				header.textContent = `â†•ï¸ Set Height for ${indices.length} Wall${indices.length > 1 ? 's' : ''}`;
				...
				const hInput = heightRow.createEl('input', {
					attr: { type: 'number', min: '0', max: '500', step: '5', placeholder: 'âˆž (infinite)' }
				});
				...
				const infiniteBtn = btnRow.createEl('button', { text: 'âˆž Infinite' });
			...
			const WALL_TYPES = {
				wall: { name: 'Wall', icon: 'ðŸ§±', color: '#ff4500', style: 'solid', blocksSight: true, blocksMovement: true },
				door: { name: 'Door', icon: 'ðŸšª', color: '#8B4513', style: 'door', blocksSight: true, blocksMovement: true },
				window: { name: 'Window', icon: 'ðŸªŸ', color: '#87CEEB', style: 'window', blocksSight: false, blocksMovement: true },
				secret: { name: 'Secret Door', icon: 'ðŸ”’', color: '#666666', style: 'dashed', blocksSight: true, blocksMovement: true },
				invisible: { name: 'Invisible Wall', icon: 'ðŸ‘»', color: '#cccccc', style: 'dotted', blocksSight: true, blocksMovement: true },
				terrain: { name: 'Terrain', icon: 'ðŸª¨', color: '#8B7355', style: 'solid', blocksSight: false, blocksMovement: true },
			} as const;
		...
		const panBtn = createToolBtn(commonToolGroup, 'â¬†', 'Pan', true);
		const selectBtn = createToolBtn(commonToolGroup, 'ðŸ‘†', 'Select');
		const highlightIcon = config.gridType === 'square' ? 'â¬œ' : 'â¬¡';
		const highlightBtn = createToolBtn(commonToolGroup, highlightIcon, 'Highlight');
		const poiBtn = createToolBtn(commonToolGroup, 'ðŸ“', 'Point of Interest');
		const markerBtn = createToolBtn(commonToolGroup, 'ðŸŽ¯', 'Marker');
		const drawBtn = createToolBtn(commonToolGroup, 'âœ', 'Draw');
		const rulerBtn = createToolBtn(commonToolGroup, 'ðŸ“', 'Ruler');
		const targetDistBtn = createToolBtn(commonToolGroup, 'ðŸ“', 'Token Distance');
		const aoeBtn = createToolBtn(commonToolGroup, 'ðŸ’¥', 'AoE');
		const eraserBtn = createToolBtn(commonToolGroup, 'ðŸ§¹', 'Eraser');
		...
		hexcrawlSectionHeader.createEl('span', { text: 'â–¼', cls: 'dnd-map-section-toggle' });
		...
		const terrainPaintBtn = createToolBtn(hexcrawlContent, 'ðŸŒ', hLoc(hcLang, 'toolTerrainPaint'));
		const climatePaintBtn = createToolBtn(hexcrawlContent, 'ðŸŒ¡ï¸', hLoc(hcLang, 'toolClimatePaint'));
		const setStartHexBtn = createToolBtn(hexcrawlContent, 'ðŸ“Œ', hLoc(hcLang, 'toolSetStartHex'));
		const hexDescBtn = createToolBtn(hexcrawlContent, 'ðŸ“', hLoc(hcLang, 'toolHexDesc'));
		...
		for (const def of TERRAIN_DEFINITIONS) {
			...
			const btn = terrainPicker.createEl('button', {
				...
				attr: { title: `${tName} â€” Speed Ã—${def.travelModifier}` },
			});
		...
		clearTerrainBtn.createEl('span', { text: 'ðŸ—‘ï¸' });
		...
		descSettingsBtn.createEl('span', { text: 'âš™ï¸' });
		...
		clearClimateBtn.createEl('span', { text: 'ðŸ—‘ï¸' });
		...
		const openHexcrawlBtn = createToolBtn(hexcrawlContent, 'ðŸ“‹', hLoc(hcLang, 'toolOpenPanel'));
		...
		hexcrawlSectionHeader.addEventListener('click', () => {
			...
			hexcrawlSectionHeader.querySelector('.dnd-map-section-toggle')!.textContent = hexcrawlSectionOpen ? 'â–¼' : 'â–¶';
		});
		...
		visionSectionHeader.createEl('span', { text: 'â–¼', cls: 'dnd-map-section-toggle' });
		const visionContent = toolbarContent.createDiv({ cls: 'dnd-map-section-content' });

		// â”€â”€ Background Edit-View filter chips â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
		...
		const bgViews: { key: BackgroundEditView; icon: string; tip: string }[] = [
			{ key: 'all',               icon: 'ðŸ‘',  tip: 'Show All' },
			{ key: 'walls',             icon: 'ðŸ§±', tip: 'Walls' },
			{ key: 'lights',            icon: 'ðŸ’¡', tip: 'Lights' },
			{ key: 'fog',               icon: 'ðŸŒ«ï¸', tip: 'Fog' },
			{ key: 'elevation',         icon: 'â›°ï¸', tip: 'Elevation' },
			{ key: 'difficult-terrain', icon: 'ðŸŒ¿', tip: 'Difficult Terrain' },
			{ key: 'env-assets',        icon: 'ðŸ“¦', tip: 'Env Assets' },
		];
		...
		const fogBtn = createToolBtn(visionContent, 'ðŸŒ«ï¸', 'Fog');
		const wallsBtn = createToolBtn(visionContent, 'ðŸ§±', 'Walls');
		const lightsBtn = createToolBtn(visionContent, 'ðŸ’¡', 'Lights');
		const elevationPaintBtn = createToolBtn(visionContent, 'â›°ï¸', 'Tile Elevation');
		const difficultTerrainBtn = createToolBtn(visionContent, 'ðŸŒ¿', 'Difficult Terrain');
		const envAssetBtn = createToolBtn(visionContent, 'ðŸ“¦', 'Env Assets');
		...
		// === TOKEN VISION TOGGLE (always visible, all layers â€” hidden for hexcrawl) ===
		...
		tokenVisionSectionHeader.createEl('span', { text: 'â–¼', cls: 'dnd-map-section-toggle' });
		...
		const refreshVisionSelector = () => {
			...
			// "All Players" option
			buildVisionOption(visionMenu, 'ðŸ‘¥', 'All Players', '');

			// Token options
			visionTokens.forEach((m: any) => {
				const markerDef = this.markerLibrary.getMarker(m.markerId);
				const icon = markerDef?.type === 'player' ? 'ðŸ‘¤' : markerDef?.type === 'creature' ? 'ðŸ‘¹' : 'ðŸ§‘';
			...
			if (selectedVisionTokenId === null) {
				visionSelected.createEl('span', { text: 'ðŸ‘¥', cls: 'dnd-map-vision-item-icon' });
				...
				if (selMarker) {
					...
					const selIcon = selDef?.type === 'player' ? 'ðŸ‘¤' : selDef?.type === 'creature' ? 'ðŸ‘¹' : 'ðŸ§‘';
			...
			// Add dropdown arrow
			visionSelected.createEl('span', { text: 'â–¾', cls: 'dnd-map-vision-arrow' });
		};
		...
		this.registerEvent(
			this.app.workspace.on('initiative-tracker:save-state' as any, (state: any) => {
				...
				// Only auto-toggle for player or friendly creatures â€” skip enemies
				...
				if (creaturePath && typeof creaturePath === 'string') {
					const noteFile = this.app.vault.getAbstractFileByPath(creaturePath);
					if (noteFile instanceof TFile) {
						const noteCache = this.app.metadataCache.getFileCache(noteFile);
		...
		tunnelsSectionHeader.createEl('span', { text: 'â–¼', cls: 'dnd-map-section-toggle' });
		const tunnelsContent = toolbarContent.createDiv({ cls: 'dnd-map-section-content' });
		
		const clearTunnelsBtn = createToolBtn(tunnelsContent, 'ðŸ§¹', 'Clear Tunnels');
		...
		setupSectionHeader.createEl('span', { text: 'â–¼', cls: 'dnd-map-section-toggle' });
		const setupContent = toolbarContent.createDiv({ cls: 'dnd-map-section-content' });
		
		const moveGridBtn = createToolBtn(setupContent, 'âœ¥', 'Move Grid');
		const calibrateBtn = createToolBtn(setupContent, 'âš™', 'Calibrate');
		const measureBtn = createToolBtn(setupContent, 'ðŸ“', 'Measure');
		
		// === PLAYER VIEW (full-width, prominent) ===
		const viewGroup = toolbarContent.createDiv({ cls: 'dnd-map-tool-group' });
		const viewBtn = createToolBtn(viewGroup, 'ðŸ“º', 'Player View', false, true);
		...
		fogModeBtn.createEl('span', { text: 'ðŸ‘ï¸' });
		fogModeBtn.addEventListener('click', (e) => {
			...
			if (iconEl) iconEl.textContent = fogMode === 'reveal' ? 'ðŸ‘ï¸' : 'ðŸš«';
		...
		const fogShapes: { shape: 'brush' | 'circle' | 'rect' | 'polygon'; icon: string; label: string }[] = [
			{ shape: 'brush', icon: 'ðŸ–Œï¸', label: 'Brush (drag)' },
			{ shape: 'circle', icon: 'â­•', label: 'Circle' },
			{ shape: 'rect', icon: 'â¬œ', label: 'Rectangle' },
			{ shape: 'polygon', icon: 'â¬ ', label: 'Polygon' },
		];
		...
		fogRevealAllBtn.createEl('span', { text: 'â˜€ï¸' });
		fogRevealAllBtn.addEventListener('click', (e) => {
			...
			new Notice('Fog cleared â€” entire map revealed');
		});
		const fogHideAllBtn = fogPicker.createEl('button', {
			cls: 'dnd-map-aoe-shape-btn dnd-fog-action-btn',
			attr: { title: 'Hide All' }
		});
		fogHideAllBtn.createEl('span', { text: 'ðŸŒ‘' });
		...
		// Magic Wand â€” auto-detect walls from image (interactive tool)
		const magicWandBtn = wallsPicker.createEl('button', {
			cls: 'dnd-map-aoe-shape-btn dnd-fog-action-btn',
			attr: { title: 'Magic Wand â€” Click dark areas to auto-detect walls' }
		});
		magicWandBtn.createEl('span', { text: 'ðŸª„' });
		...
		const mwInfo = mwSettingsDiv.createEl('div', { text: 'ðŸª„ Click dark areas on the map to generate walls', cls: 'setting-item-description' });
		...
		clearWallsBtn.createEl('span', { text: 'ðŸ—‘ï¸' });
		...
		// Wall Light button (special â€” switches to line-draw mode)
		const wallLightBtn = lightsPicker.createEl('button', {
			cls: 'dnd-map-aoe-shape-btn',
			attr: { title: 'Wall Light â€” Click two points to draw a light strip (15ft bright + 15ft dim)' }
		});
		wallLightBtn.createEl('span', { text: 'ðŸ“' });
		...
		clearLightsBtn.createEl('span', { text: 'ðŸŒ‘' });
		...
		clearElevationBtn.createEl('span', { text: 'ðŸ—‘ï¸' });
		...
		pvFullscreenBtn.createEl('span', { text: 'ðŸ–µ' });
		...
		pvCalibrateBtn.createEl('span', { text: 'ðŸŽ¯' });
		...
		const aoeShapes: { shape: 'circle' | 'cone' | 'square' | 'line'; icon: string; label: string }[] = [
			{ shape: 'circle', icon: 'â­•', label: 'Circle' },
			{ shape: 'cone', icon: 'ðŸ”º', label: 'Cone' },
			{ shape: 'square', icon: 'â¬œ', label: 'Square' },
			{ shape: 'line', icon: 'âž–', label: 'Line' },
		];
		...
		calibrateBtn.addEventListener('click', () => {
			...
			new GridCalibrationModal(
				...
				async (gs, gw, gh) => {
					...
					gridLinkBtn.textContent = gridWHLinked ? 'ðŸ”—' : 'ðŸ”“';
		...
		playerViewBtn.innerHTML = 'ðŸ‘ï¸ Player View';
      ...
      // (rAF-coalesced: many calls per frame â†’ one actual sync at next paint)
        ...
        if (this._playerMapViews && this._playerMapViews.size > 0) {
          ...
          const payload: any = {
            ...
            selectedVisionTokenId: (() => {
              ...
              return null; // creature/NPC without Show to Players â†’ All Players for player view
            })(),
      ...
      // called within one frame (e.g. mousemove â†’ redraw + sync), the actual
      ...
      (viewport as any)._syncPlayerView = () => {
		...
		sidePanelBtn.addEventListener('click', async () => {
			...
			if (viewer) {
				...
				content.createSpan({ cls: 'dnd-map-placeholder-icon', text: 'ðŸ“Œ' });
		...
		const eraseAtPoint = (px: number, py: number): boolean => {
			...
			// Helper: Get effective grid size (fixed single grid â€” no pace scaling)
			...
			const getElevationColor = (elevation: number, alpha: number = 0.35): string => {
				if (elevation < 0) {
					...
					// Above ground: green â†’ yellow â†’ orange â†’ red
			...
			const redrawAnnotations = () => {
				...
				// â”€â”€ Viewport culling: compute visible rect in canvas coords â”€â”€
				...
				// â”€â”€ Helper: compute door-wall segments for env-asset doors â”€â”€
				...
				if (config.envAssets && config.envAssets.length > 0) {
					...
					sorted.forEach((inst: EnvAssetInstance) => {
						...
						// â”€â”€ Env-asset view highlight outline â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
						...
						// â”€â”€ Selection frame & transform handles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
						if (selectedEnvAssetInstanceId === inst.id && (activeTool === 'env-asset' || activeTool === 'select') && envAlpha >= 1) {
							...
							ctx.textAlign = 'center';
							ctx.textBaseline = 'middle';
							ctx.fillText('â†»', 0, rotY);

							// â”€â”€ Pivot handle (yellow dot for non-sliding doors) â”€â”€
							if (def.category === 'door' && inst.doorConfig && inst.doorConfig.behaviour !== 'sliding') {
								const pivot = inst.doorConfig.customPivot || { x: 0, y: 0.5 };
								const pvX = (pivot.x - 0.5) * inst.width;
								const pvY = (pivot.y - 0.5) * inst.height;
				...
				// â”€â”€ Draw door-wall segments from env-asset doors (visible in wall view) â”€â”€
					...
					if (oCtx) {
						...
						for (let i = 0; i < mwMask.length; i++) {
							if (mwMask[i]) {
								...
								oData.data[i * 4 + 3] = 60;  // A â€” subtle tint
							}
						}
				...
				if (config.activeLayer === 'Background' && config.lightSources && config.lightSources.length > 0) {
					...
					config.lightSources.forEach((light: any, idx: number) => {
						...
						if (isActive) {
								...
								// â”€â”€ Wall Light (line source) â”€â”€
...
// Draw dashed selection rectangle to show clickable area
							if (light.type === 'walllight' && light.start && light.end) {
								...
								ctx.fillText('ðŸ“', light.x, light.y);
								ctx.globalAlpha = la(1.0);
							} else {
        ...
        try {
          ...
          if (gmRect && gmRect.w && gmRect.h) {
            ...
            ctx.restore();
				...
				if (rulerStart && rulerEnd) {
					...
					if (rulerVerticalFeet > 0) {
						const arrow = rulerEndElev > rulerStartElev ? 'â†‘' : 'â†“';
			...
			const applyTileElevation = (marker: any) => {
				...
				if (tileElev > 0) {
					...
					// Ground level tile â€” clear ground-induced elevation
				...
				// Update layer assignment based on elevation â€” only override
			...
			const drawMovementPath = (
				...
				if (climbDist > 0) {
					// 3D Pythagorean: sqrt(horizontalÂ² + verticalÂ²), rounded to nearest grid unit
				...
				// Distance label â€” big, near the current position
			...
			const drawAoeAffectedSquares = (
				...
				const sampleRes = 8; // sample points per cell axis (8Ã—8 = 64 samples per cell)
				...
				for (let col = colStart; col <= colEnd; col++) {
					for (let row = rowStart; row <= rowEnd; row++) {
						...
						if (hits >= total * 0.5) {
							// This cell is at least 50% covered â€” highlight it
							ctx.fillStyle = color;
			...
			const drawFogOfWar = (ctx: CanvasRenderingContext2D, w: number, h: number, isPlayerView: boolean) => {
				...
				if (isPlayerView && config.lightSources && config.lightSources.length > 0) {
					...
					config.lightSources.forEach((light: any, fogLightIdx: number) => {
						...
						if (totalRadiusPx > 0) {
							// â”€â”€ Wall Light: sample along line â”€â”€
							if (light.start && light.end && light.type === 'walllight') {
								const dx = light.end.x - light.start.x;
								const dy = light.end.y - light.start.y;
								const length = Math.sqrt(dx * dx + dy * dy);
							...
							// â”€â”€ Point Light: single gradient â”€â”€
							else {
          ...
          new Notice('Player View Mode: Drag to position, Q/E or [/] to rotate 90Â°', 4000);
        } else if (tool === 'eraser') {
      ...
      // Used to compute scale that fits the rotated rectangle into the player viewport
			...
			viewport.addEventListener('mousedown', (e: MouseEvent) => {
				...
				if (activeTool === 'pan') {
					...
					if (!foundMarker && canInteractEnvAssets) {
						...
						if (!foundEnvAsset) {
							...
							if (hitInst) {
								...
								// Clicked empty space â€” deselect env asset
								if (selectedEnvAssetInstanceId) {
									selectedEnvAssetInstanceId = null;
									redrawAnnotations();
								}
					...
					if (!campaignFolder) {
						new Notice('âš ï¸ Unable to determine campaign folder. Note or map must be in ttrpgs/[campaign-name]/ structure.');
						return;
					}
            ...
            if ((viewport as any)._syncPlayerView) (viewport as any)._syncPlayerView();				
					...
					if (targetDistState === 'selecting-origin') {
						...
						if (foundIdx >= 0) {
							...
							// Clicked empty space â€” clear measurement
							targetDistOriginIdx = -1;
							targetDistTargetIdx = -1;
							targetDistState = 'selecting-origin';
							redrawAnnotations();
						...
						// Third click â€” clear measurement
						targetDistOriginIdx = -1;
						targetDistTargetIdx = -1;
						targetDistState = 'selecting-origin';
						redrawAnnotations();
					...
					// â”€â”€ Env Asset: click to place, select, drag, or transform â”€â”€
					...
					// 2) Check if clicking on an instance body â†’ start drag or select
					const hitInst = findEnvAssetAtPoint(mapPos.x, mapPos.y);
					if (hitInst) {
					...
					// Add point to wall chain â€” snap to existing wall endpoints within 8px
					let snapX = mapPos.x;
					let snapY = mapPos.y;
					...
					try {
						...
						if (result.walls.length > 0) {
							...
							this.saveMapAnnotations(config, el);
							if ((viewport as any)._syncPlayerView) (viewport as any)._syncPlayerView();
							new Notice(`ðŸª„ ${result.walls.length} wall segments created`);
						} else {
							new Notice('No walls detected â€” try clicking a dark area or adjusting threshold');
						}
						redrawAnnotations();
					} catch (err) {
						console.error('[MagicWand] Detection failed:', err);
						new Notice('Magic wand detection failed â€” see console for details');
					}
					...
					new HexDescriptionEditModal(
						...
						(newDesc) => {
							...
							if (idx >= 0) {
								...
								// No terrain entry yet â€” create one with default 'plains'
          ...
          if (!isMovingGmRect) {
            ...
            try {
              if ((this as any)._playerMapViews && (this as any)._playerMapViews.size > 0) {
                ...
                if (firstView && firstView.mapContainer) {
                  ...
                  currentRotation = 0; // Always start at 0Â°, GM can rotate with Q/E keys
			...
			viewport.addEventListener('mousemove', (e: MouseEvent) => {
						...
						if (activeTunnel && activeTunnel.path.length > 0) {
							...
							if (snappedX !== lastPoint.x || snappedY !== lastPoint.y) {
								...
								if (activeTunnel.path.length >= 2) {
									...
									// Dot product < 0 means moving backwards (angle > 90Â°)
				...
				const dyAoe = draggedMarker.position.y - prevY;
					...
					// â”€â”€ Env asset drag / transform in progress (select or env-asset tool) â”€â”€
					...
					if (inst) {
						if (envAssetTransformHandle === 'rotate' && envAssetTransformStart && envAssetRotateStart !== null) {
							...
							// Normalise to 0â€“1 and clamp within the asset bounds
					...
					// Update wall preview position â€” snap to existing wall endpoints
		...
		viewport.addEventListener('dblclick', (e: MouseEvent) => {
			...
			viewport.addEventListener('mouseup', (e: MouseEvent) => {
				...
				if (activeTool === 'pan' && isDragging) {
					...
					// â”€â”€ Env Asset: finish drag or transform â”€â”€
					envAssetTransformHandle = null;
						...
						if (activeTunnel && activeTunnel.path.length > 0) {
							...
							if (snappedX !== lastPoint.x || snappedY !== lastPoint.y) {
								...
								if (activeTunnel.path.length >= 2) {
									...
									// Dot product < 0 means angle > 90Â°, which is moving backwards
									const dotProduct = prevDx * newDx + prevDy * newDy;
									if (dotProduct < 0) {
          ...
          if (rect) {
			...
			viewport.addEventListener('contextmenu', (e: MouseEvent) => {
				...
				// â”€â”€ Env Asset context menu (check before markers) â”€â”€
					...
					if (dist <= r) {
						...
						if (mDef && ['player', 'npc', 'creature'].includes(mDef.type)) {
							...
							const lightOptions: { type: LightSourceType; icon: string; label: string }[] = [
								{ type: 'candle', icon: 'ðŸ•¯ï¸', label: 'Candle (5ft)' },
								{ type: 'torch', icon: 'ðŸ”¥', label: 'Torch (20ft)' },
								{ type: 'lantern', icon: 'ðŸ®', label: 'Lantern (30ft)' },
								{ type: 'light', icon: 'âœ¨', label: 'Light Spell (20ft)' },
								{ type: 'daylight', icon: 'â˜€ï¸', label: 'Daylight (60ft)' },
								{ type: 'fluorescent', icon: 'ðŸ’¡', label: 'Fluorescent (30ft)' },
							];
							...
							borderHeader.innerHTML = 'ðŸŽ¨ Appearance';
							...
							elevationHeader.innerHTML = 'â†•ï¸ Elevation';
							...
							burrowCheckbox.addEventListener('change', () => {
								...
								if (burrowCheckbox.checked && depthValue > 0) {
									// Enable burrowing â€” create tunnel entrance
									m.elevation.isBurrowing = true;
									m.elevation.leaveTunnel = true;
									...
									this.saveMapAnnotations(config, el);
									if ((viewport as any)._syncPlayerView) (viewport as any)._syncPlayerView();
									new Notice('Burrowing enabled â€” tunnel path will be tracked');
									...
									// Disable burrowing â€” deactivate tunnel but keep depth
									delete m.elevation.isBurrowing;
									delete m.elevation.leaveTunnel;
									...
									this.saveMapAnnotations(config, el);
									if ((viewport as any)._syncPlayerView) (viewport as any)._syncPlayerView();
									new Notice('Burrowing disabled â€” depth retained (diving/submerged)');
								}
							});
							...
							if (isNearTunnelAccess || isInTunnel) {
								...
								tunnelTraverseHeader.innerHTML = 'ðŸ”¦ Tunnel Navigation';
							...
							auraHeader.innerHTML = 'ðŸ”µ Token Auras';
							...
							addAuraBtn.innerHTML = '<span>âž•</span> Add Aura';
						...
						deleteOption.innerHTML = `<span>ðŸ—‘ï¸</span> Delete`;
					...
					for (let i = config.lightSources.length - 1; i >= 0; i--) {
						...
						if (hitLight) {
							...
							toggleOption.innerHTML = isActive 
								? `<span>ðŸ”†</span> Active (click to extinguish)`
								: `<span>ðŸ’¡</span> Inactive (click to ignite)`;
							...
							const lightTypes: { type: LightSourceType; icon: string; name: string }[] = [
								{ type: 'candle', icon: 'ðŸ•¯ï¸', name: 'Candle (5ft)' },
								{ type: 'torch', icon: 'ðŸ”¥', name: 'Torch (20ft)' },
								{ type: 'lantern', icon: 'ðŸ®', name: 'Lantern (30ft)' },
								{ type: 'bullseye', icon: 'ðŸ”¦', name: 'Bullseye (60ft)' },
								{ type: 'light', icon: 'âœ¨', name: 'Light Spell (20ft)' },
								{ type: 'dancing', icon: 'ðŸ’«', name: 'Dancing Lights (10ft dim)' },
								{ type: 'daylight', icon: 'â˜€ï¸', name: 'Daylight (60ft)' },
								{ type: 'fluorescent', icon: 'ðŸ’¡', name: 'Fluorescent (30ft)' },
							];
							...
							if (light.cone) {
								...
								const directions = [
									{ label: 'â†‘', angle: -90, title: 'North' },
									{ label: 'â†’', angle: 0, title: 'East' },
									{ label: 'â†“', angle: 90, title: 'South' },
									{ label: 'â†', angle: 180, title: 'West' },
								];
								...
								const sliderLabel = sliderContainer.createEl('span', { text: 'ðŸ”„' });
								...
								const angleDisplay = sliderContainer.createEl('span', { text: `${light.direction || 0}Â°` });
...
// Custom colour picker for all light sources
						...
						if (dist <= wallClickRadius) {
							...
							if (wallType === 'door') {
								...
								toggleOption.innerHTML = isOpen 
									? `<span>ðŸšª</span> Close Door`
									: `<span>ðŸšª</span> Open Door`;
								toggleOption.addEventListener('click', () => {
									saveToHistory();
									wall.open = !isOpen;
									redrawAnnotations();
							...
							heightHeader.textContent = 'â†•ï¸ Wall Height';
							...
							const infiniteBtn = heightRow.createEl('button', { text: 'âˆž', cls: 'mod-cta' });
			...
			const zoomOut = zoomContainer.createEl('button', { text: 'âˆ’', cls: 'dnd-map-zoom-btn' });
			...
			const itImportBtn = controls.createEl('button', {
				text: 'âš”ï¸ Import Encounter Tokens',
				cls: 'dnd-map-toggle-btn'
			});
			itImportBtn.addEventListener('click', async () => {
				// Color name â†’ hex mapping for border colors
				...
				if (!initiativeTracker) {
					new Notice('âš ï¸ Initiative Tracker plugin not found');
					return;
				}
				...
				if (!itState || !Array.isArray(itState.creatures) || itState.creatures.length === 0) {
					new Notice('âš ï¸ No encounter loaded in Initiative Tracker');
					return;
				}
				...
				for (const [baseName, instances] of creaturesByName) {
					...
					for (let i = 0; i < instances.length; i++) {
						...
						// 1. If creature display has "(ColorName)" â†’ use that color
						// 2. If multiple of same creature â†’ auto-assign from color palette
						...
						// Try token_id lookup first (precise â€” handles same-name PCs across campaigns)
						...
						if (!markerDef) {
							...
							const icon = isPlayer ? 'ðŸ›¡ï¸' : (isFriendly ? 'ðŸ§‘' : 'ðŸ‘¹');
				...
				const msg = addedCount > 0
					? `âœ… Added ${addedCount} token${addedCount > 1 ? 's' : ''} from encounter` + (skippedCount > 0 ? ` (${skippedCount} skipped)` : '')
					: `â„¹ï¸ All encounter tokens already on the map (${skippedCount} skipped)`;
				new Notice(msg);
			});

			// Clear drawings button
			const clearBtn = controls.createEl('button', {
				text: 'ðŸ—‘ï¸ Clear Drawings',
				cls: 'dnd-map-toggle-btn'
			});
			...
			deleteBtn.addEventListener('click', () => {
				new DeleteMapConfirmModal(
					...
					() => {
						el.empty();
						el.createEl('div', {
							text: 'ðŸ—‘ï¸ Map has been deleted.',
			...
			el.createEl('div', { 
				text: `âš ï¸ Error rendering map: ${error instanceof Error ? error.message : String(error)}`,
				cls: 'dnd-map-error'
			});
		}
	}
...
if (wikiMatch && wikiMatch[1]) {
		...
		if (!filePath.endsWith('.md')) {
			...
			if (!encounterFile) {
				el.createEl('div', { 
					text: 'âš ï¸ Encounter file not found',
			...
			if (!fm || fm.type !== 'encounter') {
				el.createEl('div', { 
					text: 'âš ï¸ Not a valid encounter note',
					cls: 'dnd-encounter-block-error'
				});
				return;
			}
			...
			const nameLink = header.createEl('a', { 
				text: `âš”ï¸ ${fm.name || encounterFile.basename}`,
			...
			if (diff) {
				...
				statsRow.createEl('span', { 
					text: `ðŸ‘¥ ${diff.party_count} PCs (Lvl ~${Math.round(diff.party_avg_level || 0)})`,
					cls: 'dnd-encounter-stat'
				});
				
				// Enemy info
				statsRow.createEl('span', { 
					text: `ðŸ‘¹ ${diff.enemy_count} enemies`,
					cls: 'dnd-encounter-stat'
				});
				
				// Rounds estimate
				statsRow.createEl('span', { 
					text: `â±ï¸ ~${diff.rounds_to_defeat} rounds`,
					cls: 'dnd-encounter-stat'
				});
			}
			...
			if (creatures.length > 0) {
				...
				const creatureList = creatures.map((c: any) => 
					`${c.count || 1}Ã— ${c.name}${c.cr ? ` (CR ${c.cr})` : ''}`
				).join(', ');
			...
			loadBtn.addEventListener('click', async () => {
				...
				try {
					if (initiativeTracker.tracker?.new) {
						...
						new Notice(`âœ… Loaded: ${encounterName}`);
					}
					(this.app as any).commands?.executeCommandById("initiative-tracker:open-tracker");
				} catch (e) {
					new Notice(`âš ï¸ Could not load encounter: ${(e as Error).message}`);
				}
			});
...
/**
	...
	private async _flushMapSave(mapId: string) {
		...
		try {
			...
			const mapData = {
				...
				markers: config.markers || [],
				drawings: config.drawings || [],
				// aoeEffects intentionally omitted â€” session-only, not persisted
				tunnels: config.tunnels || [],
				poiReferences: config.poiReferences || [],
				hexTerrains: config.hexTerrains || [],
				hexClimates: config.hexClimates || [],
	...
	private async migrateExistingTemplatesToNotes(): Promise<void> {
		try {
			...
			if (migrated > 0) {
				new Notice(`ðŸ—ï¸ Migrated ${migrated} battlemap template${migrated > 1 ? 's' : ''} to ${folder}/`);
			}
		} catch (err) {
		}
	}
	 ...
	 */
	private async enrichTokenCampaigns(): Promise<void> {
		try {
			...
			for (const file of allFiles) {
				...
				const marker = this.markerLibrary.getMarker(tokenId);
				if (!marker) continue;
	...
	drawGridOverlay(container: HTMLElement, img: MapMediaElement, config: any, offsetX: number = 0, offsetY: number = 0, reuseCanvas?: HTMLCanvasElement | null): HTMLCanvasElement {
		...
		if (reuseCanvas) {
			// Reuse existing canvas â€” skip DOM create/remove/append
			canvas = reuseCanvas;
	...
	async importAllSRDData() {
		...
		new Notice(`âœ… SRD import complete! ${totalSuccess} items imported, ${totalErrors} errors. (${duration}s)`);
	}
	...
	async importSRDCreatureTokens(): Promise<{ imported: number; errors: number }> {
		...
		new Notice("ðŸ‰ Starting SRD creature token importâ€¦");
		const startTime = Date.now();
		...
		try {
			...
			if (monsters.length === 0) {
				new Notice("âš ï¸ No monsters returned from the SRD API.");
				return { imported: 0, errors: 0 };
			}

			new Notice(`ðŸ“‹ Found ${monsters.length} SRD creatures. Importingâ€¦`);
			...
			for (let i = 0; i < monsters.length; i += BATCH_SIZE) {
				...
				const batchResults = await Promise.allSettled(
					batch.map(async (entry) => {
						try {
							...
							// â”€â”€ Build / update MarkerDefinition (token) â”€â”€
				...
				if (i > 0 && i % 40 === 0) {
					new Notice(`ðŸ‰ Progress: ${i}/${monsters.length} creaturesâ€¦`);
				}
			}

			const duration = Math.round((Date.now() - startTime) / 1000);
			new Notice(
				`âœ… SRD creature import complete! ${imported} tokens imported, ${errors} errors. (${duration}s)`
			);
		} catch (error) {
			console.error("[SRD Import] Fatal error during creature import:", error);
			new Notice("âŒ SRD creature import failed. Check the console for details.");
		}
	...
	private buildSRDCreatureNote(m: any, tokenId: string, imagePath?: string): string {
		...
		// â”€â”€ Proficiencies â†’ saves & skills â”€â”€
		let saves = "";
		let skillsaves = "";
		...
		// â”€â”€ Damage & condition fields â”€â”€
		const join = (arr: any) => {
		...
		// â”€â”€ Traits / actions / legendary / reactions â”€â”€
...
\`\`\`\n\n`;
	...
	async importSRDCategory(
		...
		try {
			...
			if (!isBulkImport) {
				new Notice(`âœ… ${categoryName} import complete! ${successCount} items imported, ${errorCount} errors.`);
			}

			return { success: successCount, errors: errorCount };
		} catch (error) {
			new Notice(`âŒ Failed to import ${categoryName}: ${error instanceof Error ? error.message : String(error)}`);
